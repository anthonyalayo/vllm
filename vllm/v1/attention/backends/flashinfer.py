...
class FlashInferBackend(AttentionBackend):
    ...
    def supports_kv_cache_dtype(cls, kv_cache_dtype: CacheDType | None) -> bool:
        if kv_cache_dtype is not None and kv_cache_dtype.startswith("nvfp4"):
            # NVFP4KV-SM120: consumer Blackwell serves NVFP4 KV through the
            # FlashInfer FA2 native path (PR #49891 rebase).
    ...
    def supported_kv_cache_layouts(cls) -> tuple[KVCacheLayout, ...] | None:
        ...
        # NVFP4KV-SM120: the FA2 NVFP4 path uses the HND cache layout so
        # reshape_and_cache_flash sees contiguous strides. FP8/auto on SM12x
        # retain the upstream layout preference.
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        if self.use_fa2_nvfp4_kv:
            # NVFP4KV-SM120: neither XQA nor trtllm-gen read NVFP4 KV here;
            # decode (including speculative verification) uses native FA2.
            self.use_trtllm_decode_attention = False
            self.flashinfer_trtllm_api_decode_kernel = None
        if (
            self.use_dcp
    ...
    def get_q_data_type(self, is_prefill: bool) -> torch.dtype:
        ...
        if cache_dtype.startswith("nvfp4"):
            if current_platform.is_device_capability_family(120):
                # NVFP4KV-SM120: FA2 takes BF16/FP16 Q, dequantizes NVFP4 KV
                return self.model_config.dtype
        ...
        if self._prefill_wrapper is None:
            if self.use_dcp:
                ...
                if self.has_sinks and (
                    ...
                    # NVFP4 on SM100 requires trtllm-gen; SM120 uses FA2.
